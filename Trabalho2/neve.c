// gcc neve.c -lglut -lGL -lGLU -lm && ./a.out
...
struct s_pf {
  float x, y, z, veloc_x, veloc_y, gravity, size;
...
void InitFloco(int i){
    ...
    // int angle = (rand()%40) +250; //varia de 250 à 290
...
void DrawGLScene(){
  ...
  for(i=0;i<NUM_PARTICLES;i++) {
      ...
      //caso a vida tenha acabado ou chegou em baixo da tela,
      //começar de novo
      if(particles[i].y<-2){ 
        InitFloco(i);
      }
